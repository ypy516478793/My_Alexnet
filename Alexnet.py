...
print('Training set', mnist.train.image.shape, mnist.train.labels.shape)
print('Test set', mnist.test.image.shape, mnist.test.labels.shape)
...
batch_size = 128
display_step = 50
training_iters = mnist.train.image.shape[0] // batch_size
...
# Evaluate model
with tf.name_scope("Evaluate model"):
...
with tf.Session() as sess:
    ...
    for epoch in range(epoch_num):
        for step in range(training_iters):
            ...
            train_writer.add_summary(summary, epoch * training_iters + step)
            ...
            if step % display_step == 0:
                ...
                summary, loss, acc = sess.run([merged, cost, accuracy], feed_dict={x: batch_x,
                                                                  y: batch_y,
                                                                  keep_prob: 1.})
                valid_writer.add_summary(summary, epoch * training_iters + step)
